...
from supplychain.models.requisition import Requisition, RequisitionApproval, RequisitionItem
...
class RequisitionViewsTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.second_product, _ = Product.objects.get_or_create(
            name="Pen", defaults={"unit_cost": "1.00", "uom": cls.uom}
        )
        cls.supplier = Supplier.objects.create(name="General Supplier")
    ...
    def setUp(self):
        ...
        self.second_product = self.__class__.second_product
    ...
    def test_create_requisition_and_items(self):
        ...
        post = {
            "urgent": "",
            "evidence": SimpleUploadedFile("evidence.txt", b"proof"),
    ...
    def test_approver_queries_and_requester_updates(self):
        rq = Requisition.objects.create(
            ...
            notes="Initial",
            urgent=False,
            evidence="requisition_evidence/initial.txt",
        )
        ...
        post = {
            "urgent": "",
            "evidence": SimpleUploadedFile("clarified.txt", b"proof"),
            "destination": str(self.destination_supplier.id),
            "supplier": str(self.supplier.id),
            "Supplier_destination_sub_category": str(self.subcat.id),
            "notes": "Clarified",
            ...
            f"{prefix}-0-id": str(item.id),
            f"{prefix}-0-product": str(self.product.id),
            f"{prefix}-0-quantity": "3.00",
        }
    ...
    def test_update_removes_line_item_when_marked_delete(self):
        rq = Requisition.objects.create(
            ...
            evidence="requisition_evidence/to-delete.txt",
        )
        item1 = RequisitionItem.objects.create(requisition=rq, product=self.product, quantity="1")
        item2 = RequisitionItem.objects.create(requisition=rq, product=self.second_product, quantity="2")
        ...
        post = {
            "urgent": "",
            "evidence": SimpleUploadedFile("remove-one.txt", b"proof"),
            "destination": str(self.destination_supplier.id),
            "supplier": str(self.supplier.id),
            "Supplier_destination_sub_category": str(self.subcat.id),
            "notes": "Removing one",
            ...
            f"{prefix}-MIN_NUM_FORMS": "0",
            f"{prefix}-MAX_NUM_FORMS": "1000",
            f"{prefix}-0-id": str(item1.id),
            f"{prefix}-0-product": str(self.product.id),
            f"{prefix}-0-quantity": "1",
            f"{prefix}-1-id": str(item2.id),
            f"{prefix}-1-product": str(self.second_product.id),
            f"{prefix}-1-quantity": "2",
