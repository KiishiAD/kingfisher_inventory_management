...
def render_key_value_pdf(
    *,
    ...
    story = [Paragraph(clean_text(title), styles["Title"])]
    story.append(Paragraph(timezone.localtime(timezone.now()).strftime("Generated %Y-%m-%d %H:%M"), styles["Normal"]))
    story.append(Spacer(1, 8))

    for heading, values in sections:
        story.append(Paragraph(clean_text(heading), styles["Heading2"]))
        data = [[_paragraph(label, normal), _paragraph(value, normal)] for label, value in values]
    ...
    for heading, headers, rows in tables or []:
        ...
        if rows:
            ...
            data = [[_paragraph("No records", normal)]]
        table = Table(data, repeatRows=1)
        table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1f4e79")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
